<template>
  <div class="room">
        ...
        <li v-for="data in chats">
          <div class="chatItem">
            ...
            <div :class="data.role[0]">
              <img :src="data.headImg" alt="头像" width="50px"/>
            </div>
    ...
    <div class="footer">
      <textarea rows="6" placeholder="我也来说一句..." @click="clear" @keyup.enter="send" v-model="chatTemplate.content"></textarea>
      <div class="btn">
...
export default {
  ...
  data(){
    ...
    send(){
      ...
      // 聊天内容
      var chat = {username: this.userId,date: this.date,headImg: 'http://q2.qlogo.cn/headimg_dl?dst_uin=1607847510&spec=100',content: this.chatTemplate.content,role: ['headImg right','right','userInfo messageRight','chatBody active right']}
    ...
    clear(){
      // 清空聊天框内容
      if(this.chatTemplate.content === "我也来说一句~"){
        this.chatTemplate.content = "";
      }
    }
  },
  mounted() {
    ...
    ws.onopen=res => {
      ...
      let a = {username: '系统信息',date: yyyy+'-'+MM+'-'+dd+' '+hh+':'+mm+':'+ss,headImg: 'http://q1.qlogo.cn/g?b=qq&nk=1607847510&s=640',content: userId+'进入聊天室',role: ['none','center','userInfo','sysInfo']}
...
.headImg{
  ...
  border-radius: 50px;
  background-color: #c6c4c4;
  box-shadow: 5px 5px 5px #cbced1,-5px -5px 5px white;
}
.userInfo{
  color: #807e7e;
  position: relative;
